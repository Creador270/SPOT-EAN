...
for name in OLD_MOTOR_NAMES:
    if "shoulder" in name:
        OLD_MOTOR_LIMITS_BY_NAME[name] = [-0.547, 0.547]
    elif "leg" in name:
        OLD_MOTOR_LIMITS_BY_NAME[name] = [-2.59, 1.547]
    elif "foot" in name:
        OLD_MOTOR_LIMITS_BY_NAME[name] = [-0.1, 2.589]
...
class Spot(object):
  ...
  """
    ...
    def __init__(self,
                 pybullet_client,
                 urdf_root= urdf, # pybullet_data.getDataPath(),
                 time_step=0.01,
                 action_repeat=1,
        ...
        if reload_urdf:
            if self._self_collision_enabled:
                self.quadruped = self._pybullet_client.loadURDF(
                    urdf,# pybullet_data.getDataPath() + "/assets/urdf/spot.urdf",
                    init_position,
    ...
    """
        ...
        self._pybullet_client.resetJointState(
            self.quadruped,
            self._joint_name_to_id["joint_" + leg_position + "_leg"],
            self.INIT_POSES[self._pose_id][3 * leg_id],
            targetVelocity=0)

        self._pybullet_client.resetJointState(
            self.quadruped,
            self._joint_name_to_id["joint_" + leg_position + "_shoulder"],
            self.INIT_POSES[self._pose_id][3 * leg_id + 1],
            targetVelocity=0)
        self._pybullet_client.resetJointState(
            self.quadruped,
            self._joint_name_to_id["joint_" + leg_position + "_foot"],
        ...
        if self._accurate_motor_model_enabled or self._pd_control_enabled:
            ...
            self._pybullet_client.setJointMotorControl2(
                bodyIndex=self.quadruped,
                jointIndex=(self._joint_name_to_id["joint_" + leg_position +
                                                   "_leg"]),
